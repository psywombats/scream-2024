    ...
    public override void OnInspectorGUI()
    {
        ...
        if (tileset != null && tool == SelectionTool.Paint)
        {
            ...
            for (int y = tileset.size.y - 1; y >= 0; y -= 1)
            {
                ...
                {
                    Rect selectRect = EditorGUILayout.BeginHorizontal(GUILayout.Width(PxPerTile * tilesetScale), GUILayout.Height(PxPerTile * tilesetScale));
                    Tile tile = tileset.GetTile<Tile>(new Vector3Int(x, y, 0));

                    GUILayout.Box("", style, GUILayout.Width(PxPerTile * tilesetScale), GUILayout.Height(PxPerTile * tilesetScale));
