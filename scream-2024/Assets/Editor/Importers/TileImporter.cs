...
public class TileImporter : AssetPostprocessor
{
    ...
    public void OnPreprocessTexture()
    {
        ...
        importer.spritePivot = new Vector2(TacticsTerrainEditor.PxPerTile / 2, TacticsTerrainEditor.PxPerTile / 2);
        importer.isReadable = true;

        List<SpriteMetaData> newSheet = new List<SpriteMetaData>();
        for (int y = 0; y < textureSize.y / TacticsTerrainEditor.PxPerTile; y += 1)
        {
            for (int x = 0; x < textureSize.x / TacticsTerrainEditor.PxPerTile; x += 1)
            {
                int index = y * (textureSize.y / TacticsTerrainEditor.PxPerTile) + x;
                SpriteMetaData data = new SpriteMetaData();
                data.rect = new Rect(
                    x * TacticsTerrainEditor.PxPerTile,
                    (textureSize.y / TacticsTerrainEditor.PxPerTile - y - 1) * TacticsTerrainEditor.PxPerTile,
                    TacticsTerrainEditor.PxPerTile,
                    TacticsTerrainEditor.PxPerTile);
    ...
    public static void CreateTiles(string assetPath)
    {
        ...
        foreach (Object spriteObject in spriteObjects)
        {
            ...
            var tile = AssetDatabase.LoadAssetAtPath<PropertiedTile>(spritePath);
            if (tile == null)
            {
                tile = ScriptableObject.CreateInstance<PropertiedTile>();
    ...
    public static void CreatePalette(string assetPath)
    {
        ...
        if (palette == null)
        {
            ...
            grid.cellSize = new Vector3(1, 1, 0.0f);
    ...
    public static void PopulatePalette(string assetPath)
    {
        ...
        for (int y = 0; y < tex.height / TacticsTerrainEditor.PxPerTile; y += 1)
        {
            for (int x = 0; x < tex.width / TacticsTerrainEditor.PxPerTile; x += 1)
            {
                Vector3Int pos = new Vector3Int(x, (tex.height / TacticsTerrainEditor.PxPerTile - y - 1), 0);
                TileBase existingTile = map.GetTile<TileBase>(pos);

                string tileName = NameForTile(name, x, y);
                var newTile = AssetDatabase.LoadAssetAtPath<PropertiedTile>(tileDirectory + "/" + tileName + ".asset");
